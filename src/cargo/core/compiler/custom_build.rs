...
pub struct BuildOutput {
    ...
    pub rerun_if_changed: Vec<Utf8PathBuf>,
...
pub struct BuildDeps {
    ...
    pub build_script_output: Utf8PathBuf,
...
impl BuildOutput {
    ...
    pub fn parse_file(
        ...
        pkg_descr: &str,
        script_out_dir_when_generated: &Utf8Path,
        script_out_dir: &Utf8Path,
        nightly_features_allowed: bool,
        targets: &[Target],
        msrv: &Option<RustVersion>,
    ) -> CargoResult<BuildOutput> {
        let contents = paths::read_bytes(path.as_std_path())?;
        BuildOutput::parse(
            &contents,
            library_name,
            pkg_descr,
    ...
    pub fn parse(
        ...
        for line in input.split(|b| *b == b'\n') {
            ...
            let value = value.replace(
                script_out_dir_when_generated.as_str(),
                script_out_dir.as_str(),
            );
            ...
            match key {
                ...
                "rustc-link-search" => library_paths.push(Utf8PathBuf::from(value)),
                ...
                "rerun-if-changed" => rerun_if_changed.push(Utf8PathBuf::from(value)),
    ...
    pub fn parse_rustc_flags(
        value: &str,
        whence: &str,
    ) -> CargoResult<(Vec<Utf8PathBuf>, Vec<String>)> {
        ...
        while let Some(flag) = flags_iter.next() {
            if flag.starts_with("-l") || flag.starts_with("-L") {
                ...
                match flag {
                    ...
                    "-L" => library_paths.push(Utf8PathBuf::from(value)),
...
impl BuildDeps {
    ...
    pub fn new(output_file: &Utf8Path, output: Option<&BuildOutput>) -> BuildDeps {
        BuildDeps {
...
fn prev_build_output(
    build_runner: &mut BuildRunner<'_, '_>,
    unit: &Unit,
) -> (Option<BuildOutput>, Utf8PathBuf) {
    let script_out_dir = build_runner.files().build_script_out_dir(unit);
    let script_run_dir = build_runner.files().build_script_run_dir(unit);
    let root_output_file = script_run_dir.join("root-output");
    let output_file = script_run_dir.join("output");

    let prev_script_out_dir = paths::read_bytes(root_output_file.as_std_path())
